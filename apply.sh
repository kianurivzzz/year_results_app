...
original_author="kianurivzzz"
original_name="year_results_app"
original_urlname="year_results_app"
original_description="Awesome year_results_app created by kianurivzzz" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/year_results_app "${name}"
...
then
    ...
    echo "then run 'year_results_app' or 'python -m year_results_app'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'year_results_app' or 'python -m year_results_app' or 'poetry run year_results_app'"
fi
