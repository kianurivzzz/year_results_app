...
- 🎯 Entry points to execute your program using `python -m <year_results_app>` or `$ year_results_app` with basic CLI argument parsing.
...
---
# year_results_app Flask Application

Awesome year_results_app created by kianurivzzz
...
```bash
git clone https://github.com/kianurivzzz/year_results_app year_results_app
cd year_results_app
make install
```
...
```bash
year_results_app create-db   # run once
year_results_app populate-db  # run once (optional)
year_results_app add-user -u admin -p 1234  # ads a user
year_results_app run
```
